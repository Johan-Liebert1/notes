...
func canFinishParticularCourse2(
	courseNum int,
	...
	for _, prereq := range prerequisites {
		...
		if prereq[1] == courseNum {
			if _, ok := (*visited)[prereq[0]]; ok {
				return true
			}

			// find out if we can actually complete prereq[0]
			if !canFinishParticularCourse2(prereq[0], visiting, visited, prerequisites, array) {
				return false
			}

			(*visited)[courseNum] = true
			*array = append(*array, prereq[0])
		}
	}
